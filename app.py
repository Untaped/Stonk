...
PREDICTIONS_CSV = os.getenv('PREDICTIONS_CSV', "stonk_download/predictions.csv")
...
def health_check():
    ...
    try:
        # Check data files
        fundamentals_exists = os.path.exists(FUNDAMENTALS_CSV)
        price_history_exists = os.path.exists(PRICE_HISTORY_CSV)
        
        return jsonify({
            'status': 'healthy',
            'data_files': {
                'fundamentals': fundamentals_exists,
                'price_history': price_history_exists
            },
...
def load_csv_data():
    """Load and cache CSV data with enhanced error handling"""
    ...
    if (_cache_timestamp is None or 
        current_time - _cache_timestamp > 300 or
        _fundamentals_cache is None or 
        _price_history_cache is None):
        
        print("🔄 Loading CSV data...")
        
        # Load fundamentals with error handling
        try:
            if os.path.exists(FUNDAMENTALS_CSV):
                ...
                print(f"⚠️ Fundamentals CSV not found at: {FUNDAMENTALS_CSV}")
        except Exception as e:
            print(f"❌ Error loading fundamentals: {e}")
            _fundamentals_cache = pd.DataFrame()
        
        # Load price history with error handling
        try:
            if os.path.exists(PRICE_HISTORY_CSV):
                ...
                print(f"⚠️ Price history CSV not found at: {PRICE_HISTORY_CSV}")
        except Exception as e:
...
def get_sp500_symbols():
    ...
    try:
        ...
        print(f"Error getting symbols from CSV: {e}")
...
def get_fundamentals(symbol, date=None):
    """Get fundamental data from CSV with better error handling"""
    try:
        fundamentals_df, _ = load_csv_data()
        ...
        if 'symbol' not in fundamentals_df.columns:
            raise ValueError(f"Symbol column not found in fundamentals data")
...
def get_price_history_csv(symbol, timeframe='2Y'):
    """Get price history from CSV with enhanced error handling"""
    try:
        ...
        if 'symbol' not in price_history_df.columns:
            print("Symbol column not found in price history data")
            return {
                "dates": [],
                "prices": [],
                "volumes": [],
        ...
        print(f"Error in get_price_history_csv for {symbol}: {e}")
        return
...
if __name__ == '__main__':
    print("Starting Flask app without authentication...")
